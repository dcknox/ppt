...
effects.ci_025 <- as.data.frame(acte.frechet$posterior$effects.quantiles[,,'quantile_0.025'])
effects.ci_975 <- as.data.frame(acte.frechet$posterior$effects.quantiles[,,'quantile_0.975'])

plot_effects$min.ci_025 <- effects.ci_025$min
plot_effects$min.ci_975 <- effects.ci_975$min
plot_effects$max.ci_025 <- effects.ci_025$max
plot_effects$max.ci_975 <- effects.ci_975$max

plot_effects$naive <- acte.frechet$naive$effects$naive
plot_effects$naive.ci_025 <- acte.frechet$posterior$naive_effects.quantiles[,'naive', 'quantile_0.025']
plot_effects$naive.ci_975 <- acte.frechet$posterior$naive_effects.quantiles[,'naive', 'quantile_0.975']
...
colnames(plot_ate) <- c('ctrl','treat','ate','ate.ci_025','ate.ci_975')
plot_ate$C <- 0
...
acte.frechet$naive.ci$min <-
    acte.frechet$posterior$naive_effects.quantiles[, 'naive', 'quantile_0.025']
acte.frechet$naive.ci$max <-
    acte.frechet$posterior$naive_effects.quantiles[, 'naive', 'quantile_0.975']
acte.frechet$naive.ci$point <- NULL
...
acte.frechet$effects.ci <-
    as.data.frame(acte.frechet$posterior$effects.quantiles[,, 'quantile_0.025'])
acte.frechet$effects.ci$max <-
    acte.frechet$posterior$effects.quantiles[, 'max', 'quantile_0.975']
acte.frechet$effects.ci$treatment <-
    factor(treatment.labels[acte.frechet$effects.ci$treat %.%
                            acte.frechet$effects.ci$ctrl],
           levels = treatment.labels)
...
## but estimated sensitivity interval equal estimated bounds for rho = 0.17
as.matrix(acte.frechet$sensitivity$strata[
...
effects.ci_975 <-
    as.data.frame(acte.binary$posterior$effects.quantiles[,,'quantile_0.975'])
...
plot_effects$naive.ci_025 <-
    acte.binary$posterior$naive_effects.quantiles[,'naive', 'quantile_0.025']
plot_effects$naive.ci_975 <-
    acte.binary$posterior$naive_effects.quantiles[,'naive', 'quantile_0.975']
...
ggplot(plot_effects, aes(x = C, colour = type, linetype = type)) +
    ...
    scale_y_continuous(breaks = seq(-.5, .5, .25),
                       limits = c(-.65, .65)) +
    ggtitle('Discuss Story with Friends\n(binary)')
dev.off()
...
## bounds on treatment effects
acte.binary$effects.ci <-
    as.data.frame(acte.binary$posterior$effects.quantiles[,, 'quantile_0.025'])
acte.binary$effects.ci$max <-
