...
bounds.binary.calc = function(p_s, p_c.s, p_sc_pool,
    ...
    ## out$min_ca = out$min_ca[,,ind,drop=TRUE]
    ## out$max_ca = out$max_ca[,,ind,drop=TRUE]
    ## out$effects.min = out$effects.min[,ind,,drop=TRUE]
    ## out$effects.max = out$effects.max[,ind,,drop=TRUE]
...
bounds.binary = function(data,
    ...
    if (posterior){
        ...
        ##     array(NA,
        ##           dim = c(n_MC,rep(J,2)),
        ##           dimnames=list(draw=NULL,S="S." %.% 1:J,C="C." %.% 1:J))

        ## min_ca_MC = max_ca_MC =
        ##     array(NA,
        ##           dim = c(n_MC,rep(J,2)),
        ##           dimnames=list(draw=NULL,C="C." %.% 1:J,A="A." %.% 1:J))
        ...
        for (i in 1:n_MC){
            ...
            ## max_ca_MC[i,,] <- bounds_star$max_ca
            ## min_ca_MC[i,,] <- bounds_star$min_ca
            ## effects_star = melt(bounds_star$effects.min)
            ## effects_star$C = as.numeric(effects_star$C)
            ## colnames(effects_star)[colnames(effects_star)=="value"] = "min"
            ## effects_star$max = melt(bounds_star$effects.max)$value
            ## effects_star$ctrl = effects.ind[effects_star$effect,"ctrl"]
            ## effects_star$treat = effects.ind[effects_star$effect,"treat"]
            ## effects_star$effect = NULL
            ## effects_MC[i,,] <- as.matrix(effects_star)
