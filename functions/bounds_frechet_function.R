...
#' patient preference trial with design described in XXX (anonymized for review)
#'
    ...
    for (s in 1:J){
        for (c in 1:J){
            for (a in (1:J)[-c]){
                ...
                ##     knots = attr(F_diff_Y_sa[[s,a]], "knots")
                ##     min_sca.var[s,c,a] =
                ##         (integrate_outer(F_Y_sa[[s,a]], from=y.min, to=Q_b, knots) / (n_sa_force[s,a] - 1) +
                ##          integrate_outer(F_Y_scc[[s,a]], from=y.min, to=Q_b, knots) / (n_sc[s,a] - 1) * p_c.s[s,a]^2) *
                ##         2 / p_c.s[s,c]^2
                ##     max_sca.var[s,c,a] =
                ##         (integrate_outer(F_Y_sa[[s,a]], from=Q_1mb, to=y.max, knots) / (n_sa_force[s,a] - 1) +
                ##          integrate_outer(F_Y_scc[[s,a]], from=Q_1mb, to=y.max, knots) / (n_sc[s,a] - 1) * p_c.s[s,a]^2) *
                ##         2 / p_c.s[s,c]^2

                ##     if (b < .5){
                ##         ## covariance when lower and upper bounds integrals involve disjoint domains
                ##         minmax_sca.cov[s,c,a] =
                ##             ((y.max - Q_1mb) - integrate_ecdf(F_Y_sa[[s,a]], from=Q_1mb, to=y.max, knots)) * # inner
                ##             integrate_ecdf(F_Y_sa[[s,a]], from=y.min, to=Q_b, knots) / (n_sa_force[s,a] - 1) / p_c.s[s,c]^2 + # outer
                ##             ((y.max - Q_1mb) - integrate_ecdf(F_Y_scc[[s,a]], from=Q_1mb, to=y.max, knots)) * # inner
                ##             integrate_ecdf(F_Y_scc[[s,a]], from=y.min, to=Q_b, knots) / (n_sc[s,a] - 1) * p_c.s[s,a]^2 / p_c.s[s,c]^2 # outer
                ##     } else {
                ##         ## covariance when lower and upper bounds integrals involve overlapping domains
                ##         minmax_sca.cov[s,c,a] =
                ##             ((y.max - Q_1mb) - integrate_ecdf(F_Y_sa[[s,a]], from=Q_1mb, to=y.max, knots)) * # inner
                ##             integrate_ecdf(F_Y_sa[[s,a]], from=y.min, to=Q_1mb, knots) / (n_sa_force[s,a] - 1) / p_c.s[s,c]^2 + # outer
                ##             ((y.max - Q_1mb) - integrate_ecdf(F_Y_scc[[s,a]], from=Q_1mb, to=y.max, knots)) * # inner
                ##             integrate_ecdf(F_Y_scc[[s,a]], from=y.min, to=Q_1mb, knots) / (n_sc[s,a] - 1) * p_c.s[s,a]^2 / p_c.s[s,c]^2 + # outer
                ##             integrate_outer(F_Y_sa[[s,a]], from=Q_1mb, to=Q_b, knots) * 2 / (n_sa_force[s,a] - 1) / p_c.s[s,c]^2 + # both
                ##             integrate_outer(F_Y_scc[[s,a]], from=Q_1mb, to=Q_b, knots) * 2 / (n_sc[s,a] - 1) * p_c.s[s,a]^2 / p_c.s[s,c]^2 + # both
                ##             ((y.max - Q_b) - integrate_ecdf(F_Y_sa[[s,a]], from=Q_b, to=y.max, knots)) * # inner
                ##             integrate_ecdf(F_Y_sa[[s,a]], from=Q_1mb, to=Q_b, knots) / (n_sa_force[s,a] - 1) / p_c.s[s,c]^2 + # outer
                ##             ((y.max - Q_b) - integrate_ecdf(F_Y_scc[[s,a]], from=Q_b, to=y.max, knots)) * # inner
                ##             integrate_ecdf(F_Y_scc[[s,a]], from=Q_1mb, to=Q_b, knots) / (n_sc[s,a] - 1) * p_c.s[s,a]^2 / p_c.s[s,c]^2 # outer
                ##     }
                ## }
            }
...
sens.calc = function(min_ca,
    ...
    J = nrow(p_c.s)
    ## max_ca = apply(max_sca * p_s.c %o% rep(1,J), c(2,3), sum)
    ## min_ca = apply(min_sca * p_s.c %o% rep(1,J), c(2,3), sum)
...
bounds.frechet = function(data,
    ...
    ##   - check if tbl_df & cast to df, b/c length(tbl_df$var) returns 1
    ##   - auto-drop NAs in Y
    ...
    ## !any(is.na(d[d$D == 1,-which(colnames(d) == 'C')])) # no missing data in
    ## forced condition, except choice
